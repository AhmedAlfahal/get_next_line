...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/17 21:41:14 by aalfahal          #+#    #+#             */
/*   Updated: 2023/01/02 14:55:29 by aalfahal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_lst(t_list **lst)
{
	...
	while ((*lst) != NULL)
	{
		...
		if (line_len(tmp) != -1)
		{
			result = join(result, cut(tmp, 0, line_len(tmp) + 1));
			add_front(lst, lst_new(cut(tmp, line_len(tmp) + 1, len(tmp))));
